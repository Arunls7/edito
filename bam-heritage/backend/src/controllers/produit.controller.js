...
const getAll = async (req, res) => {
  try { res.json(await prisma.produit.findMany({ where: { actif: true } })); }
  catch { res.status(500).json({ error: 'Erreur serveur' }); }
};

const getAllAdmin = async (req, res) => {
  try { res.json(await prisma.produit.findMany()); }
  catch { res.status(500).json({ error: 'Erreur serveur' }); }
};
...
const create = async (req, res) => {
  try {
    ...
    const produit = await prisma.produit.create({ data: { nom, description, prix, stock, imageUrl, categorie, stripeId: stripePrice.id } });
    res.status(201).json(produit);
  } catch (err) { res.status(400).json({ error: err.message }); }
};

const update = async (req, res) => {
  try { res.json(await prisma.produit.update({ where: { id: req.params.id }, data: req.body })); }
  catch { res.status(404).json({ error: 'Produit introuvable' }); }
};
