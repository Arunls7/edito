  ...
  try {
    ...
    const [membres, total] = await Promise.all([
      prisma.user.findMany({
        where,
        select: { id: true, email: true, nom: true, prenom: true, role: true, createdAt: true, abonnements: { orderBy: { createdAt: 'desc' }, take: 1 } },
        skip: Number(skip), take: Number(limit), orderBy: { createdAt: 'desc' },
      }),
...
const updatePopupConfig = async (req, res) => {
  try {
    let config = await prisma.popupConfig.findFirst();
    if (!config) {
      config = await prisma.popupConfig.create({ data: req.body });
    } else {
      config = await prisma.popupConfig.update({ where: { id: config.id }, data: req.body });
    }
    res.json(config);
  } catch {
    res.status(500).json({ error: 'Erreur serveur' });
