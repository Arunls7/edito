const champions = [
  { nom: 'Tony Yoka', titres: ['Champion Olympique Rio 2016', 'Champion de France Pro', 'Champion d\'Europe WBA'], discipline: 'BOXE PRO', nbTitres: 3, pro: true },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[2px]">
          {champions.map((c) => (
            <div key={c.nom} className="card-noir p-6 relative overflow-hidden group">
              <span className="absolute bottom-2 right-3 font-bebas text-[5rem] text-white/[0.04] leading-none select-none pointer-events-none">{c.nbTitres}</span>
              <div className="flex items-center gap-2 mb-4">
                {c.pro
                  ? <span className="bg-rouge text-white font-inter text-[10px] uppercase tracking-widest px-2 py-0.5">Pro</span>
                  : <span className="border border-white/40 text-white/60 font-inter text-[10px] uppercase tracking-widest px-2 py-0.5">Amateur</span>
                }
              ...
              <ul className="space-y-2">
                {c.titres.map((t) => (
                  <li key={t} className="flex items-start gap-2">
                    <div className="w-[3px] h-3 bg-rouge flex-shrink-0 mt-1" />
                    <span className="font-inter text-xs text-white/50 leading-tight">{t}</span>
                  </li>
                ))}
              </ul>
            </div>
