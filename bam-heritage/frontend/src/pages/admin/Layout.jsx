...
const navItems = [
  ...
  { to: '/admin/evenements', label: 'Evenements' },
  { to: '/admin/produits', label: 'Produits' },
  { to: '/admin/popup', label: 'Popups' },
];
    ...
    <div className="min-h-screen bg-noir flex flex-col">
      <div className="bg-noir-dark border-b border-rouge/30 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="bg-rouge text-white font-inter text-[10px] uppercase tracking-widest px-2 py-0.5">Admin</span>
        </div>
        ...
        <aside className="w-56 bg-noir-dark border-r border-white/10 flex flex-col p-6 gap-1">
          ...
          {navItems.map((item) => (
            <NavLink key={item.to} to={item.to} end={item.exact}
              className={({ isActive }) => `font-inter text-sm px-3 py-2.5 transition-colors ${
                isActive ? 'text-white bg-rouge/10 border-l-[3px] border-rouge pl-[calc(0.75rem-3px)]' : 'text-white/40 hover:text-white hover:bg-white/5'
              }`}>
              {item.label}
            </NavLink>
          ))}
